...
optparser.add_option("-T", "--train", default= "{path}/data/eng.train".format(path = base_path), help="Training dataset path")
optparser.add_option("-d", "--dev", default= "{path}/data/eng.testa".format(path = base_path), help="Validation dataset path")
optparser.add_option("-t", "--test", default= "{path}/data/eng.testb".format(path = base_path), help="Test dataset path")
optparser.add_option("--test_train", default= "{path}/data/eng.train_small".format(path = base_path), help="Test-train dataset path")
optparser.add_option("--score", default="{path}/evaluation/temp/score.txt".format(path = base_path), help="score file location")
...
optparser.add_option(
    "-p",
    "--pre_emb",
    default= "{path}/embeddings/glove.6B.100d.txt".format(path=base_path),
...
optparser.add_option(
    ...
    default="0.5",
    type="float",
    help="Dropout on the input (0 = no dropout)",
)
...
if __name__ == "__main__":
    ...
    # checks pre-trained embedding for existing words and adds to word dict
    all_word_embeds = {}
